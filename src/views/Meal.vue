<template>
  <section
    v-if="data"
    class=" meal-recipe container"
  >
    <div class="sum">
      <h1>{{data.meal.strMeal}}</h1>
      <p>{{data.meal.strArea}} - {{data.meal.strCategory}} </p>
    </div>
    <div class="line"></div>
    <div class="meal">
    ...
    <div>
      ...
      <iframe
        width="420"
        height="315"
        :src="changeUrlYoutube(data.meal.strYoutube)"
        frameborder="0"
        allowfullscreen
      ></iframe>
    </div>
    ...
    getMealId(id) {
      mealService
        ...
        .then((response) => {
          ...
          this.findAllMeasure(response.data.meals[0]);
        })
        .catch((error) => {
          console.log(error);
        });
